...
{
	struct fs_superblock check;
	uint8_t buffer[FS_BLOCKSIZE];
	if (fs_ata_read_block(0, buffer) < 0)
		return -1;
	memcpy(&check, buffer, sizeof(struct fs_superblock));
	if (check.magic == FS_MAGIC) {
		memcpy(&super, &check, sizeof(struct fs_superblock));
		return 0;
	}
	return -1;
}
...
static int create_superblock()
{
	...
	superblock_num_blocks = CONTAINERS(sizeof(struct fs_superblock), FS_BLOCKSIZE);
	available_blocks = num_blocks - superblock_num_blocks;
	free_blocks = (uint32_t) ((double) (available_blocks) /
			(1.0 +
			 (double) (sizeof(struct fs_inode) + .125)/(4.0 * FS_BLOCKSIZE) +
			 .125/(FS_BLOCKSIZE)
			 )
			);
	total_inodes = free_blocks / 8;
	total_bits = free_blocks;
	inode_sector_size = CONTAINERS((total_inodes * sizeof(struct fs_inode)), FS_BLOCKSIZE);
	...
	return fs_ata_write_block(0, wbuffer);
}
